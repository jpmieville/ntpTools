...
// Client represents an SNTP client.
type Client struct {
	server  string
	version uint8
...
func NewClient(server string, version uint8, timeout time.Duration) *Client {
	return &Client{
		server:  server,
		version: version,
		timeout: timeout,
...
func (c *Client) createNTPRequest() []byte {
...
func (c *Client) FetchTime() (*NTPResult, error) {
...
func PrintResult(result *NTPResult) {
	fmt.Println()
